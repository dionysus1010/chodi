...
import lessons from "../../../lessons/javascript_lesson.json";
...
export default function JavaScriptCourses() {
  ...
  useEffect(() => {
   ...
   const checkCertificates = async () => {
    try {
      const response = await fetch(`/api/certificates?userId=${userId}&courseId=javascript`);
    ...
    useEffect(() => {
      const checkCertificate = async () => {
        if (userId && javascriptLessons.length > 0) {
          try {
            const response = await fetch(`/api/certificates?userId=${userId}&courseId=javascript`);
            if (response.ok) {
      ...
      checkCertificate();
    }, [userId, progress, javascriptLessons.length]);
  ...
  const handleGenerateCertificate = async () => {
    try {
      ...
      a.href = url;
      a.download = `javascript_certificate.pdf`;
  ...
  <button
    style={{
      ...
      backgroundColor:
        hasCertificate || (javascriptLessons.length > 0 && Object.values(progress).filter(Boolean).length === javascriptLessons.length)
    ...
    disabled={
      !hasCertificate &&
      (javascriptLessons.length === 0 || Object.values(progress).filter(Boolean).length !== javascriptLessons.length)
    }
    title={
      javascriptLessons.length > 0 && Object.values(progress).filter(Boolean).length !== javascriptLessons.length
        ? `Complete ${javascriptLessons.length - Object.values(progress).filter(Boolean).length} more lessons to unlock`
        : ''
    }
  >
  ...
  {!hasCertificate &&
    Object.values(progress).filter(Boolean).length === javascriptLessons.length &&
    javascriptLessons.length > 0 && (
      <div style={styles.certificateBanner}>
        <div style={styles.certificateTitle}>Course Completed!</div>
        <div style={styles.certificateText}>Congratulations! You've completed all javascript lessons.</div>
      </div>
    )}
</div>
