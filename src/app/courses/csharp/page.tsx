...
import CsharpEditor from "../../components/editors/CSharpEditor/CSharpEditor";

import lessons from "../../../lessons/csharp_lesson.json";
...
export default function CsharpCourses() {
  const [csharpLessons, setCsharpLessons] = useState<Lesson[]>([]);
  ...
  useEffect(() => {
    ...
    if (!storedUser) {
      Swal.fire({
        icon: 'warning',
        title: 'Access Denied',
        text: 'Please login first to access C++ Course',
  ...
  useEffect(() => {
    const loadLessonsAndProgress = async () => {
      const csharpLessons = lessons.map((lesson: Lesson) => ({
        ...lesson,
        courseId: 'csharp'
      }));
      setCsharpLessons(csharpLessons);
      if (csharpLessons.length > 0) setSelectedLesson(csharpLessons[0]);

      if (userId) {
        try {
          const response = await fetch(`/api/progress?userId=${userId}&courseId=csharp`);
  ...
  const checkCertificates = async () => {
    try {
      const response = await fetch(`/api/certificates?userId=${userId}&courseId=csharp`);
    ...
    useEffect(() => {
      const checkCertificate = async () => {
        if (userId && csharpLessons.length > 0) {
          try {
            const response = await fetch(`/api/certificates?userId=${userId}&courseId=csharp`);
            if (response.ok) {
      ...
      checkCertificate();
    }, [userId, progress, csharpLessons.length]);
  ...
  const handleGenerateCertificate = async () => {
    try {
      const response = await fetch('/api/certificates', {
        ...
        body: JSON.stringify({ userId, courseId: 'csharp' })
      });
      ...
      a.href = url;
      a.download = `csharp_certificate.pdf`;
  ...
  const progressPercentage = csharpLessons.length > 0
    ? (Object.values(progress).filter(Boolean).length / csharpLessons.length) * 100
    : 0;
      ...
      <div style={styles.mainContainer}>
        ...
        <div style={{
          ...
          <div style={styles.sidebarHeader}>
            <h2 style={styles.sidebarTitle}>C++ Course</h2>
            ...
            <button 
              ...
              onClick={async () => {
                ...
                const { isConfirmed } = await Swal.fire({
                  title: 'Are you sure?',
                  text: "This will reset your C++ progress!",
                  icon: 'warning',
                ...
                if (isConfirmed) {
                  try {
                    ...
                    Swal.fire({
                      icon: 'success',
                      title: 'Reset!',
                      text: 'Your C++ progress has been reset.',
                      timer: 2000
                    });
                  } catch (error) {
                    Swal.fire({
  ...
  <button
    style={{
      ...
      backgroundColor:
        hasCertificate || (csharpLessons.length > 0 && Object.values(progress).filter(Boolean).length === csharpLessons.length)
    ...
    disabled={
      !hasCertificate &&
      (csharpLessons.length === 0 || Object.values(progress).filter(Boolean).length !== csharpLessons.length)
    }
    title={
      csharpLessons.length > 0 && Object.values(progress).filter(Boolean).length !== csharpLessons.length
        ? `Complete ${csharpLessons.length - Object.values(progress).filter(Boolean).length} more lessons to unlock`
        : ''
    }
  >
  ...
  {!hasCertificate &&
    Object.values(progress).filter(Boolean).length === csharpLessons.length &&
    csharpLessons.length > 0 && (
      <div style={styles.certificateBanner}>
        <div style={styles.certificateTitle}>Course Completed!</div>
        <div style={styles.certificateText}>Congratulations! You've completed all csharp lessons.</div>
      </div>
    )}
</div>
          <div style={styles.lessonList}>
            {csharpLessons.map((lesson) => (
              <div
                key={lesson.id}
                onClick={() => {
